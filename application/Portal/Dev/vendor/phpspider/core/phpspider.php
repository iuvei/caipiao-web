...
class phpspider
{
    ...
    const AGENT_MOBILE_BAIDU_SPIDER = 'Mozilla/5.0 (Linux;u;Android 4.2.2;zh-cn;) AppleWebKit/534.46 (KHTML,like Gecko) Version/5.1 Mobile Safari/10600.6.3 (compatible; Baiduspider/2.0; +http://www.baidu.com/search/spider.html)';
     ...
     * 新增数 
     */
    public static $insert_num = 0;

    /**
     * 更新数 
     */
    public static $update_num = 0;
    ...
    public function start($use_cache = true)
    {
        $this->parse_command();
    ...
    public function get_html_fields($html, $url, $page) 
    {
        ...
        if (!empty($fields)) 
        {
            ...
            {
                $fields = call_user_func($this->on_extract_page, $page, $fields, $this);
            }
    ...
    public function check_cache($use_cache = true)
    {
     ...
     * 更新数目加一
     */
    public function incr_update_num()
    {
        if (self::$use_redis)
        {
            $update_num = cls_redis::incr("update_num"); 
        }
        else 
        {
            self::$update_num++;
    ...
    public function get_update_num()
    {
        if (self::$use_redis)
        {
            $update_num = cls_redis::get("update_num"); 
        }
        else 
        {
            $update_num = self::$update_num;
        }
        return $update_num ? $update_num : 0;
    }

    /**
     * 新增数目加一
     */
    public function incr_insert_num()
    {
        if (self::$use_redis)
        {
            $insert_num = cls_redis::incr("insert_num"); 
        }
        else 
        {
            self::$insert_num++;
     ...
     * 新增数目
     */
    public function get_insert_num()
    {
        if (self::$use_redis)
        {
            $insert_num = cls_redis::get("insert_num"); 
        }
        else 
        {
            $insert_num = self::$insert_num;
