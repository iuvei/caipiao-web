...
class FastBeatController extends HomebaseController {
    ...
    public function MemberBet(){
        ...
        foreach ($data as $key => $value) {
            ...
            $add['BackBetIP']=get_client_ip(0, true);
            $add['ProfitAndLoss']=0;
            $add['UpdateDt']=$this->time;
        ...
        file_put_contents("auto/php54n/tmp/commission".$bet["BetInfoID"]."_".$this->user["id"],json_encode($agentts));

         $save=array();
        $save['PeriodsNumber']=$PeriodsNumber;
        file_put_contents('storexx/'.md5($this->user['id']).'text',json_encode($storexx));
