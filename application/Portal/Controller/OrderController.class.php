...
//        $this->uid = session('uid');
    ...
    public function index(){
        $list0=M('type')->order('sort asc')->limit(5)->select();
        $list=M('type')->order('sort asc')->select();
        $map['type']=1;
        $map['enable']='1';
        $list1=M('type')->where($map)->order('sort asc')->select();
        $map['type']=2;
        $map['enable']='1';
        $list2=M('type')->where($map)->order('sort asc')->select();
        $map['type']=3;
        $map['enable']='1';
        $list3=M('type')->where($map)->order('sort asc')->select();
    ...
    function tjbet(){
    	$this->check_login();
         ...
         foreach ($data['type'] as $key => $value) {
            ...
            $money=$money+$zqs*$data['amount'][$key]*$data['actionNum'][$key]*$data['beiShu'][$key];
         }
