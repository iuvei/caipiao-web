...
class FixTwoController extends HomebaseController {
    ...
    public function GetMemberBetOdds(){
        ...
        $data['APartialOdds']=fix_decimal($bonusprop, 4);
        $data['BOdds']=fix_decimal($bonusprop, 4);
    ...
    public function CheckCredit(){
        ...
        $data['CanUseCredit']=fix_decimal(($this->user[money]-$this->user[yymoney]), 4);
        $data['DefaultCredit']=fix_decimal(sprintf("%.1f",$this->user[money]), 4);
        $data['UsedCredit']=fix_decimal($this->user[yymoney], 4);
        $data['status']=true;
    ...
    public function SetMemberBet(){
        ...
        foreach ($listbet as $key => $value) {
           ...
           //$store=storexx($this->uid,$value['BetNumber'],$PeriodsNumber);
           //$store2=storexx2($this->uid,$played['id'],$PeriodsNumber);
