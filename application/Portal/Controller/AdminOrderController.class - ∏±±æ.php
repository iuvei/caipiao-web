...
class AdminOrderController extends AdminbaseController {
    ...
    public function xgzd(){
        ...
        $this->assign('zdxx', $zdxx);
        $this->assign('uid', $uid);
        $this->display();
    }
    public function xgzd_post(){

        $betnumber=I('post.betnumber');
        $odds=I('post.odds');
        $save1=array();
        ...
        $save2=array();
         ...
         $lhxx=json_decode($zdxx['lhxx'],true);
         $lhxx2=json_decode($bet['lhxx'],true);
         foreach($lhxx as $key=>$one){
            if($one['money']){
                if($sfzj){
                    $lhxx[$key]['win']=-$one['money']/$zdxx["BetAmount"]*($save1["ProfitAndLoss"]+$zdxx["BackComm"]);
                }
                else{
                    $lhxx[$key]['win']=-$one['money']/$zdxx["BetAmount"]*($save1["ProfitAndLoss"]+$zdxx["BackComm"]);
                }
                $lhxx2[$key]['win']=$lhxx2[$key]['win']+($lhxx[$key]['win']-$one['win']);
            }
         }
         $save1['lhxx']=json_encode($lhxx);
         $save2['lhxx']=json_encode($lhxx2);
