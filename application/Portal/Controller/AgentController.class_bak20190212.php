      ...
      if($this->agent['issubaccount']){
        ...
        if($_GET['BetInfoID'] && $_GET['uid']){
          ...
          if (isset($_GET['IsExpand']) && $_GET['IsExpand'] == 1) {
            ...
            // print_r($list);
            foreach ($list as $row) {
              $map['BetInfoID']=$row['betinfoid'];
              $uid = $row['uid'];
              // $count += M('userbet'.$uid)->where($map)->count();
              $tmp_list = M('userbet'.$uid)->where($map)->limit('0,5000')->order('id desc')->select();
              // print_r($tmp_list);
              foreach ($tmp_list as &$bet_row) {
                $bet_row['uid'] = $uid;
              }
              $new_list = array_merge($new_list, $tmp_list);
            }
            // $PageCount=ceil($count/20);
            $list = $new_list;
            $chazt=0;
