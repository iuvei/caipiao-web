   ...
   public function dologin() {
        ...
        if (!empty($result)) {
            if ($result['user_status'] != 1) {
                $this->ejson('此账号已被' . [2 => '锁住', 3 => '禁用'][$result['user_status']]);
            }
            // 逐级检查上级
            $path = get_array_unique_id_list(explode('-', $result['agentpath']));
            foreach ($path as $tmp_parent_aid) {
                $tmp_parent = M("agent")->find($tmp_parent_aid);
                if ($tmp_parent['user_status'] != 1) {
                    $this->ejson('此账号的上级已被' . [2 => '锁住', 3 => '禁用'][$tmp_parent['user_status']]);
                }
            }
    ...
    public function dologin2() {
        ...
        if (!empty($result)) {
            ...
            // 逐级检查上级
            $path = get_array_unique_id_list(explode('-', $result['path']));
            foreach ($path as $tmp_parent_aid) {
                $tmp_parent = M("agent")->find($tmp_parent_aid);
                if ($tmp_parent['user_status'] != 1) {
                    $this->ejson('此账号的上级已被' . [2 => '锁住', 3 => '禁用'][$tmp_parent['user_status']]);
            ...
            if ($pass == $result['user_pass']) {
                ...
                session('time', $time);
                $gsave = array();
                $gsave['last_login_ip'] = get_client_ip(0, true, $result);
                $gsave['last_login_time'] = date("Y-m-d H:i:s");
                session('last_login_time', $gsave['last_login_time']);
                $gsave['token']=$time;
                ...
                if($info['IsSubAccount']){
                    $info['IsChildShowReport']=$result['ischildshowreport']==1 ? true : false;
                }else{
