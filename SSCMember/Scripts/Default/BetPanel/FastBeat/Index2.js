...
define(function (require, exports, module) {
    var AccountList = function () {
        ...
        this.getMemberBetOdds = function () {/**获取会员赔率*会默认优先拿子公司下设置的赔率***********************************/
            ...
            if (number != undefined && number.length >= 2) {
                ...
                else if (numberLenX == null || number.length == 4) {
        ...
        this.number.subscribe(function (newValue) {
            ...
            if (that.currentType() < 2 && newValue.length > 4) newValue = newValue.substring(0, 4);
            ...
            if ((that.currentType() < 2 && value.length == 4) || (that.currentType() > 1 && value.length == 5)) {
                document.getElementById("SumFastBeat").select();
        ...
        this.ConfirmTheBet = function () {
            ...
            if (typeof number === 'undefined' || number === '') {
                ...
                if (status === '1') {
                    ...
                    if ((number.length === 5 && xLen > 3) || (number.length === 4 && xLen > 2) || (number.length < 4 && !/^\d{2,4}$/.test(number))) {
                        confirmTheBetAjaxComp = true;
                        Utils.tip('号码出错,没有定位到号码', false, setFocus);
                        return;
                    } else if (that.quanZhuang() && number.length === 4) {
                        /*
                         * 4^4中除去重叠和重复
                         */
                        number = number.split('');
    ...
    AccountList.prototype.StopBetNumber = function (fn) {
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function (data) {
                that.periodslist($.isArray(data)?data:[]);
