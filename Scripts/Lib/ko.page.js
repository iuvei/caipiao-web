define(function (require, exports, module) {
        ...
        update: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
            ...
            if (config.showLinks) {
                for (var i = range[0]; i <= range[1]; i++) { 
                    if (maxPage === 1) {//如果总页数只有一条
                        html += '<button>上一页</button> <span>' + i + ' </span><button>下一页</button> '; 
                    } else {
                        if (i === current) {
                            html += '<span>' + i + ' </span>';
                        } else {
            ...
            html += '| 共' + maxPage + '页';
            html += ' | <input type="text" class="page-go-input" style="width: 40px; height: 22px;" placeholder="页码" /> <a href="javascript:;" class="page-go-submit" data-n="">Go</a>';
            div.innerHTML = html;
            element.appendChild(div);
            $(div).delegate('>.page-go-input', 'keyup', function (e) {
                $('.page-go-submit').attr('data-n', $(this).val());
            });
            $(div).delegate('>a', 'click', function (e) {
                if (e.target.tagName === 'SPAN') return;
                var page = $(this).data('n');
                if (page == '') {
                    return;
                }
                valueAccessor()(page);
    ...
    var page = function (self, callback, options) {
        ...
        this.currentPage.subscribe(function () {
            ...
            callback.call(that, currentPage);
        });
