    ...
    ko.bindingHandlers.page = {
        update: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
            ...
            if (config.first && range[0] !== current) {
                html += '<a class="button" href="javascript:;" data-n="1">' + config.first + '</a>';
            }
            if (config.prev && current > 1) {
                html += '<a class="button" href="javascript:;" data-n="' + (current - 1) + '">' + config.prev + '</a>';
            }
            if (config.showLinks) {
                for (var i = range[0]; i <= range[1]; i++) {
                    if (i === current) {
                        ...
                        html += '<a class="button" href="javascript:;" data-n="' + i + '">' + i + '</a>';
                    }
                }
            } else {
            ...
            if (config.next && current !== maxPage) {
                html += '<a class="button" href="javascript:;" data-n="' + (current + 1) + '">' + config.next + '</a>';
            }
            if (config.last && range[1] !== current) {
                html += '<a class="button" href="javascript:;" data-n="' + maxPage + '">' + config.last + '</a>';
            }
