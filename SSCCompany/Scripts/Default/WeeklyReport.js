...
define(function (require, exports, module) {
    var MonthReport = function (param) {
       ...
       // this.LevelStatus = ko.observable(-1);
        ...
        this.isWeeklyReport = ko.observable();//表示“级别”查看，不是“登录人”
        this.DateFrom = ko.observable(Utils.DateHelp.add(new Date($("#ServerDate").val()), 1 - new Date().getDay(), "day", 'yyyy-MM-dd'));//(Utils.DateHelp.add(new Date(), 1 - new Date().getDay(), "day", 'yyyy-MM-dd'));
        this.DateTo = ko.observable(Utils.DateHelp.add(new Date($("#ServerDate").val()), 7 - new Date().getDay(), "day", 'yyyy-MM-dd'));//(Utils.DateHelp.add(new Date(), 7 - new Date().getDay(), "day", 'yyyy-MM-dd'));
        this.Setday = {
            ThisWeek: function () { //本周
                var nd = new Date($("#ServerDate").val()); //new Date();
        ...
        this.Seach = function () {
            ...
            that.DateFrom($("#staDt").val());
            that.DateTo($("#endDt").val());
            that.getData();
        }
        ...
        this.goBack = function (data) {
            ...
            //    framework.view('/Report/Index', 'Report/Index', { backParams: param.backParams });
            //    return;
            //} && !param.backParams
    ...
    MonthReport.fn = MonthReport.prototype = {
        ...
        getData: function () {
            ...
            params.isWeeklyReport = that.isWeeklyReport();

            params.AgentLevel = that.AgentLevel();
            //params.LevelStatus = that.LevelStatus();

            params.DateFrom = that.DateFrom();//that.isBack() ? that.periodsTime() : new Date().format('yyyy-MM-dd');
            params.DateTo = that.DateTo();
            ...
            $.get('/index.php/portal/agent/ReportForWeekAll', params, function (data) {
                load.cancel();
                that.clearTotal();
                that.historyParams = {
                ...
                if ($.isArray(data) && data.length) {
                    ...
                    //that.LevelStatus(data[0].AgentLevel);
        ...
        next: function (data, event, isDirectMember) {
            ...
            that.companyID(that.isDirectMember() ? that.IsDirectID() == undefined ? Utils.Cookie.get('AccountID') : that.IsDirectID() : data.CompanyID);
            ...
            that.isWeeklyReport(true);
            /*that.companyID(data.CompanyID);
            that.companyType(data.CompanyType);
            that.currentLevel(data.LevelStatus);
            that.isDirectMember(isDirectMember || this.isDirectMember());*/
            that.isBack(true);
        ...
        getNext: function (params) {
            var that = this;
            params['Date'] = new Date($("#ServerDate").val()).getFullYear() + '-' + that.M.periodsTime
    ...
    */
    ...
    exports.done = function () {
        ...
        //guide_right.eq(3).css({ color: 'red' });
        //guide_right.filter("[href*='WeeklyReport']").css({ color: 'red' });
        guide_right.removeClass("menuon").filter("[href*='WeeklyReport']").addClass("menuon");
    };
